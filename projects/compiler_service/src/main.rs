...
async fn compile(
    ...
    match compile_source(&req.source, req.optimize) {
        Ok((binary, ir)) => Ok(Json(CompileResponse {
            success: true,
            job_id,
        ...
        Err(errors) => Ok(Json(CompileResponse {
            success: false,
            job_id,
            binary_size: None,
            errors,
...
async fn execute(
    ...
    info!(
        "Execute request: job_id={}, timeout={}ms",
        job_id, req.timeout_ms
    );
    ...
    match compile_source(&req.source, true) {
        ...
        Err(errors) => Ok(Json(ExecuteResponse {
            success: false,
            job_id,
            output: errors.join("\n"),
            exit_code: 1,
