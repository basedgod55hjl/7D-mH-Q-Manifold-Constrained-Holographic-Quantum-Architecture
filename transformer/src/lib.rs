...
pub use attention::Crystal7DAttentionBlock;
...
pub fn project_to_poincare(v: &[f64], curvature: f64) -> Vec<f64> {
    ...
    v.iter()
        .enumerate()
        .map(|(i, &x)| {
            let phi_weight = if i < 7 {
                PHI_BASIS[i] / PHI_BASIS[6]
            } else {
                1.0
            };
            x * scale * phi_weight
...
pub fn mobius_add(u: &[f64], v: &[f64], curvature: f64) -> Vec<f64> {
    ...
    u.iter()
        .zip(v.iter())
        .map(|(&ui, &vi)| (num_u * ui + num_v * vi) / den.max(1e-10))
        .collect()
}
...
impl Crystal7DAttention {
    pub fn new(config: Crystal7DConfig) -> Self {
        ...
        let phi_weights: Vec<f64> = (0..head_dim)
            .map(|i| {
                if i < 7 {
                    PHI_BASIS[i] / PHI_BASIS[6]
                } else {
        ...
        for h in 0..num_heads {
            for qi in 0..seq_len_q {
                for ki in 0..seq_len_k {
                    ...
                    let mut score = 0.0;
                    for d in 0..head_dim {
                        let w = if self.config.phi_attention {
                            self.phi_weights[d]
                        } else {
                            1.0
                        };
    ...
    pub fn new(dim: usize, max_seq_len: usize, theta: f64, phi_modulated: bool) -> Self {
        ...
        for pos in 0..max_seq_len {
            for i in 0..(dim / 2) {
                ...
                let mod_freq = if phi_modulated && i < 7 {
                    freq * (PHI_BASIS[i] / PHI_BASIS[6])
                } else {
                    freq
                };
