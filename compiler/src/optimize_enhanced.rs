...
use crate::ir::{IR7D, IRBlock, IRInstruction};
        ...
        for block in blocks.iter_mut() {
            ...
            while i < block.instructions.len() {
                ...
                if i + 1 < block.instructions.len() {
                    if let Some(fused_op) = self.try_fuse(&block.instructions[i], &block.instructions[i + 1]) {
                        fused.push(fused_op);
    ...
    fn run(&self, blocks: &mut Vec<IRBlock>) -> OptimizationResult {
        ...
        for block in blocks.iter_mut() {
            ...
            block.instructions.retain(|inst| {
                self.is_instruction_live(inst, &used_regs)
            });
...
impl StrengthReduction {
    fn reduce(&self, inst: &IR7D) -> Option<IR7D> {
        match inst {
            ...
            IR7D::MulFloat(x, 2.0) | IR7D::MulFloat(2.0, x) => {
                Some(IR7D::AddFloat(*x, *x))
            },
            // x * 0.5 -> x >> 1 (for integers, simplified here)
            IR7D::MulFloat(x, 0.5) => {
                Some(IR7D::DivFloat(*x, 2.0))
            },
...
impl LoopOptimization {
    fn find_loop_headers(&self, blocks: &[IRBlock]) -> Vec<usize> {
        ...
        blocks.iter()
            .enumerate()
            .filter(|(_, b)| b.label.starts_with("loop"))
            .map(|(i, _)| i)
            .collect()
    }
