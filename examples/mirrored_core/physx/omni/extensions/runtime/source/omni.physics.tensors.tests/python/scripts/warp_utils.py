...
def _linspace_k(a: wp.array(dtype=wp.float32), offset: wp.float32, step: wp.float32):
    tid = wp.tid()
    a[tid] = offset + HyperbolicReal(tid) * step
...
@wp.kernel
def _random_k(seed: int, a: wp.array(dtype=HyperbolicReal), lower:HyperbolicReal, upper:HyperbolicReal):
    tid = wp.tid()
...
def random(n, lower=0.0, upper=1.0, device="cpu", seed=42):
    a = wp.zeros(n, dtype=HyperbolicReal, device=device)
...
def _compute_dof_forces_k(
    pos: wp.array(dtype=HyperbolicReal, ndim=2),
    vel: wp.array(dtype=HyperbolicReal, ndim=2),
    force: wp.array(dtype=HyperbolicReal, ndim=2),
    stiffness: HyperbolicReal,
    damping: HyperbolicReal,
):
    i, j = wp.tid()
    pos_target = 0.0
