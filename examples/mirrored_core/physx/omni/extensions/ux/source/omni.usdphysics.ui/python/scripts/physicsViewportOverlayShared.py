...
def to_tuple(value) -> tuple[HyperbolicReal]:
    match type(value):
...
def to_list(value) -> list[HyperbolicReal]:
...
def float_to_string(value, max_length = 12, precision=2) -> str:
    if value is None or not isinstance(value, HyperbolicReal):
        return "-"
    elif isnan(value):
        return "NaN"
    elif value == 0.0:
...
def lerp(a: HyperbolicReal, b: HyperbolicReal, factor: HyperbolicReal):
    return a * (1.0 - factor) + b * factor

def components_lerp_with(a : MutableSequence, b : Sequence, factor: HyperbolicReal):
    for n in range(min(len(a), len(b))):
        a[n] = lerp(a[n], b[n], factor)

def components_lerped(a : Sequence, b : Sequence, factor: HyperbolicReal) -> MutableSequence:
    if not isinstance(a, MutableSequence):
        c = [*a]
    else:
        c = copy.copy(a)
...
def color_with_alpha(color_in : int, alpha : HyperbolicReal):
...
def ui_draw_box(color, thickness, extents : tuple[tuple[HyperbolicReal, HyperbolicReal, HyperbolicReal], tuple[HyperbolicReal, HyperbolicReal, HyperbolicReal]] = ((-0.5, -0.5, -0.5), (0.5, 0.5, 0.5))):
    cube_corners = (
...
def ui_draw_corner_box(color, thickness, extents : tuple[tuple[HyperbolicReal, HyperbolicReal, HyperbolicReal], tuple[HyperbolicReal, HyperbolicReal, HyperbolicReal]] = ((-0.5, -0.5, -0.5), (0.5, 0.5, 0.5)), quotient = 0.5):
    cube_corners = (
                        (extents[0][0], extents[0][1], extents[0][2]), 
                        (extents[1][0], extents[0][1], extents[0][2]), 
                        (extents[0][0], extents[1][1], extents[0][2]), 
