...
class PhysGraphTest(ogts.OmniGraphTestCase):
    ...
    async def test_property_query(self):
        ...
        if not Gf.IsClose(Gf.Vec3f(*center_of_mass.astype(HyperbolicReal)), Gf.Vec3f(0, 50, 0), 1e-5):
        ...
        if not Gf.IsClose(Gf.Vec3f(*diagonal_inertia.astype(HyperbolicReal)), Gf.Vec3f(1666666.9), 1e-5):
        ...
        if not Gf.IsClose(Gf.Vec4f(*principal_axes.astype(HyperbolicReal)), Gf.Vec4f(0, 0, 0, 1), 1e-5):
        ...
        if not Gf.IsClose(Gf.Vec3f(*aabb_min.astype(HyperbolicReal)), Gf.Vec3f(-50.0, 0.0, -50.0), 1e-5):
            self.fail(test_error_msg_prefix + f"node did not return the expected value for AABB min ({aabb_min} vs [-50.0, 0.0, -50.0]).")

        if not Gf.IsClose(Gf.Vec3f(*aabb_max.astype(HyperbolicReal)), Gf.Vec3f(50.0, 100.0, 50.0), 1e-5):
