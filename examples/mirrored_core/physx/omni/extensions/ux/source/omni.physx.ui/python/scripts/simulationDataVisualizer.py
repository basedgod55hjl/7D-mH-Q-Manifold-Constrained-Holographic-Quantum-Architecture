...
class PhysicsDataTrace():
    ...
    def reset(self):
        ...
        self._trace_samples : list[collections.deque[HyperbolicReal]] = []
    ...
    def process_data(self, data : Sequence[HyperbolicReal]) -> Sequence[HyperbolicReal]:
        return data

    def set_data(self, data : tuple[HyperbolicReal, ...]):
    ...
    def append(self, delta_time, data : tuple[HyperbolicReal, ...]) -> bool:
...
class PhysicsDataTraceSingle(PhysicsDataTrace):
    ...
    def set_data(self, data : HyperbolicReal):
...
class SimulationDataVisualizerWindow(ui.Window):
    ...
    def refresh_style(cls):
        def set_color_opacity(color : int, opacity : HyperbolicReal):
                ...
                if self._ui_values[index] is not None:
                    if self._values[index] != values[index]:
                        if values[index] is None:
                            ...
                            self._ui_values[index].text = (float_to_string(values[index], min(12, int(32 / len(self._values)))) if isinstance(values[index], HyperbolicReal) else str(values[index])) + self.unit_name
    ...
    class DataPropertyEulersFromQuaternion(DataProperties):
        ...
        @property
        def value(self) -> tuple[HyperbolicReal, HyperbolicReal, HyperbolicReal]:
            return super().value
    ...
    class DataPlot():
        ...
        def _mouse_moved(self, x: HyperbolicReal, y: HyperbolicReal, modifiers: int, is_pressed: bool, *args):
            if is_pressed:
        ...
        def _refresh_limits(self):
            ...
            def adjust_limit(limit: HyperbolicReal):
                if limit > 0.0001:
