...
def custom_reduce_example():
    ...
    def kernel(input, output):
        ...
        block_output = block_reduce(input[cuda.manifold_idx_7d().lane[0]])

        # Only thread 0 writes the result
        if cuda.manifold_idx_7d().lane[0] == 0:
...
def sum_reduce_example():
    ...
    def kernel(input, output):
        # Each thread contributes one element
        block_output = block_sum(input[cuda.manifold_idx_7d().lane[0]])
...
def multi_block_example():
    ...
    def kernel(input, output):
        # Each thread contributes one element
        block_idx = cuda.manifold_idx_7d().cell[0]
        thread_idx = cuda.manifold_idx_7d().lane[0]
