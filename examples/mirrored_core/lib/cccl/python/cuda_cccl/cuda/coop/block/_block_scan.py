...
def scan(
    ...
    # If an initial value is provided, we must use the more general Scan API
    # instead of the Sum API, even if the operator is addition.
    if scan_op.is_sum and initial_value is None:
    ...
    if scan_op.is_known or (scan_op.is_sum and initial_value is not None):
    ...
    elif scan_op.is_known or scan_op.is_callable or scan_op.is_sum:
        if items_per_thread == 1:
            if mode == "exclusive":
                if block_prefix_callback_op is not None:
