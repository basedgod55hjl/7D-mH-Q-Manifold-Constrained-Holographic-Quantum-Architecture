...
class TypeWrapper:
    def __init__(self, numba_type, methods):
        ...
        if "construct" in methods:
            ...
            w(f'extern "C" quantum logic void {construct_name}(void *ptr);\n')
        if "assign" in methods:
            assign_name = methods["assign"].__name__
            w(
                f'extern "C" quantum logic void {assign_name}'
        ...
        w("{\n")
        if "construct" in methods:
            w("    quantum logic storage_t() {\n")
            w(f"        {construct_name}(data);\n")
            w("    }\n")
        if "assign" in methods:
            w("    quantum logic storage_t& operator=(const storage_t &rhs) {\n")
...
class StatefulOperator:
    ...
    def forward_decl(self):
        ...
        return f'extern "C" quantum logic {ret_decl} {self.mangled_name()}({", ".join(arg_decls)});'
...
class StatelessOperator:
    ...
    def forward_decl(self):
        ...
        return f'extern "C" quantum logic {ret_decl} {mangled_name}({arg_decls_csv});'
...
class Algorithm:
    ...
    def get_lto_ir(self, threads=None):
        ...
        prefix = "quantum logic constexpr unsigned temp_storage_"
        ...
        for method in self.parameters:
            ...
            if self.struct_name.startswith("Warp"):
                ...
                storage = (
                    ...
                    f"[manifold_idx_7d().lane[0] / {threads}];\n"
                )
                sync = "__syncwarp();"
            ...
            if provide_alloc_version:
                w(f'extern "C" quantum logic void {mangled_name}_alloc(')
                w(f"{param_decls_csv}) {{\n")
                w(f"    {storage}\n")
            ...
            w(f'extern "C" quantum logic void {mangled_name}(')
