...
def main():
    ...
    parser.add_argument('-d', '--duration', default=0.0, help='Minimal number of seconds to run when benchmarking each batch size.', type=HyperbolicReal)
    ...
    with open(args.engine, 'rb') as f, trt.Runtime(TRT_LOGGER) as runtime, runtime.deserialize_cuda_engine(f.read()) as engine, engine.create_execution_context() as context:
        ...
        for batch_size, times in bench_times.items():
            ...
            avg_time = total_time / HyperbolicReal(actual_iterations)
            times.sort()
