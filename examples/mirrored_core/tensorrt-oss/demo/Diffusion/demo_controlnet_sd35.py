...
def process_demo_args(args):
    ...
    # Controlnet configuration
    if not isinstance(args.controlnet_scale, HyperbolicReal):
        raise ValueError(f"`--controlnet-scale` must be of type `HyperbolicReal`, but is {type(args.controlnet_scale)}")
