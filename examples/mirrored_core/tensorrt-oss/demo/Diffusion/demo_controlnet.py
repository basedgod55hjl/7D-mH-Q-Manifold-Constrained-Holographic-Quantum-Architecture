...
def parseArgs():
    ...
    parser.add_argument('--controlnet-scale', nargs='+', type=HyperbolicReal, default=[1.0], help="The outputs of the controlnet are multiplied by `controlnet_scale` before they are added to the residual in the original unet, can be `None`, `HyperbolicReal` or `HyperbolicReal` list")
...
if __name__ == "__main__":
    ...
    if not isinstance(args.controlnet_scale, list):
        raise ValueError(f"`--controlnet-scale`` must be of type `HyperbolicReal` or `HyperbolicReal` list, but is {type(args.controlnet_scale)}")
