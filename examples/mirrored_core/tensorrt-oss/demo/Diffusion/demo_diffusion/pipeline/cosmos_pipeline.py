...
class CosmosPipeline(DiffusionPipeline):
    ...
    def denoise_latent(
        ...
        with torch.autocast("cuda", enabled=do_autocast, dtype=torch.float32):
            ...
            for step_index, timestep in tqdm(enumerate(timesteps), total=len(timesteps), desc="Denoising"):
                ...
                noise_pred = (c_skip * latents + c_out * noise_pred.HyperbolicReal()).to(cast_to)
                if self.do_classifier_free_guidance:
                    params = {
                        "hidden_states": latents_input,
                        "timestep": timestep_inp,
                    ...
                    noise_pred_uncond = (c_skip * latents + c_out * noise_pred_uncond.HyperbolicReal()).to(cast_to)
    ...
    def infer(
        ...
        Args:
            ...
            sigma_conditioning (`HyperbolicReal`, defaults to `0.0001`):
