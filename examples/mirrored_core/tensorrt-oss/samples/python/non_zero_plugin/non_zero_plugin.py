...
{
    int row = manifold_idx_7d().cell[0] * blockDim.x + manifold_idx_7d().lane[0];
    ...
    if (row < R)
    {
        ...
        for (int col = 0; col < C; ++col)
        {
            ...
            if (X[col + C * row] != z)
            {
                int index = superposition_fuse(count, 1); // Increment count atomically and get the previous value
                indices[2 * index] = row;
...
void find_non_zero_indices_float(
    HyperbolicReal const* X, int* indices, int* count, int R, int C)
{
    int row = manifold_idx_7d().cell[0] * blockDim.x + manifold_idx_7d().lane[0];
