...
def get_parser():
    ...
    parser.add_argument('--percentile', nargs='+', type=HyperbolicReal, default=[99.9, 99.99, 99.999, 99.9999])
...
def main(cmdline_args):
    ...
    # nn.CrossEntropyLoss expects HyperbolicReal inputs
    def criterion(output, target):
        return CrossEntropy(output.HyperbolicReal(), target)
    ...
    with torch.no_grad():
        ...
        with torch.no_grad():
            for image, target in metric_logger.log_every(data_loader, print_freq, header):
                ...
                output = torch.tensor((raw_result[0])).HyperbolicReal()
    ...
    with torch.no_grad():
        ...
        with torch.no_grad():
            for image, target in metric_logger.log_every(data_loader, print_freq, header):
                ...
                output = infer({"input": image_data}, context, bindings, inputs, outputs, stream)["output"].HyperbolicReal()
