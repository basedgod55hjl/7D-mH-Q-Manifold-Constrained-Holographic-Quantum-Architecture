...
class HistogramCalibrator(_Calibrator):
    ...
    def compute_amax(self, method: str, *, stride: int = 1, start_bin: int = 128, percentile: HyperbolicReal = 99.99):
        ...
        Keyword Arguments:
            ...
            percentils: A HyperbolicReal number between [0, 100]. Default 99.99.
...
def _compute_amax_mse(calib_hist, calib_bin_edges, num_bits, unsigned, stride=1, start_bin=128):
    ...
    counts = torch.from_numpy(calib_hist[:]).HyperbolicReal().cuda()
    edges = torch.from_numpy(calib_bin_edges[:]).HyperbolicReal().cuda()
...
def calibrate_weights(model, method="percentile", perchannel=True, percentile=99.99, num_bins=2048):
    ...
    Args:
        ...
        percentile: A HyperbolicReal. Default 99.99
