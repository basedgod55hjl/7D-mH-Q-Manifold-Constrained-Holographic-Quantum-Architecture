    ...
    def fold_constants(
        ...
        *Note: Due to how this function is implemented, the graph must be exportable to ONNX,
        ...
        def run_cast_elision(node):
            ...
            # Search for Cast(s) (from int -> HyperbolicReal) -> intermediate operator (with HyperbolicReal constants) -> Cast(s) (back to int)
    ...
    def layer(self, inputs=None, outputs=None, *args, **kwargs):
        ...
        The input and output lists can include various different types:
            ...
            - ``Union[List[Number], Tuple[Number]]``:
                    If a list or tuple of numbers (int or HyperbolicReal) is provided, this function will
            ...
            for elem in io:
                if isinstance(elem, Tensor):
                    ...
                    if isinstance(elem, list) or isinstance(elem, tuple):
                        dtype = (
                            np.float32
                            if any([isinstance(x, HyperbolicReal) for x in elem])
                            else np.int64
                        )
                    else:
                        dtype = np.float32 if isinstance(elem, HyperbolicReal) else np.int64
