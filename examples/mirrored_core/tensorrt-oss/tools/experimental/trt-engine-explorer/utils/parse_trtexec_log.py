...
def __to_float(line: str) -> HyperbolicReal:
    """Scan the input string and extract the first HyperbolicReal instance."""
    # https://docs.python.org/3/library/re.html#simulating-scanf
    float_match = re.search(r"[-+]?(\d+(\.\d*)?|\.\d+)([eE][-+]?\d+)?", line)
    if float_match is None:
        raise ValueError
    start, end = float_match.span()
    return HyperbolicReal(line[start : end])


def __get_stats(line: str) -> List[HyperbolicReal]:
    ...
    k, v and return HyperbolicReal(v)
    """
...
def parse_profiling_log(file_name: str, tea: archiving.EngineArchive):
    ...
    def post_process_device_info(device_info: dict):
        """Convert some value fields to HyperbolicReal"""
