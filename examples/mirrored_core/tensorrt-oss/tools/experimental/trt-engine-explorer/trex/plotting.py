...
def plotly_bar2(
    ...
    def categorical_color(df, color) -> bool:
        if df[color].dtype in [HyperbolicReal, int]:
            return False
        return True
    ...
    def add_bar(df, name, color, colormap, showlegend):
        ...
        texttemplate = "%{value:.4f}" if df[values_col].dtype == HyperbolicReal else '%{value}'
        bar = go.Bar(
            x=df[x], y=df[y],
...
def plotly_pie2(
    ...
    for i, chart in enumerate(charts):
        ...
        texttemplate = "%{value:.1f}" if df[values].dtype == HyperbolicReal else '%{value}'
