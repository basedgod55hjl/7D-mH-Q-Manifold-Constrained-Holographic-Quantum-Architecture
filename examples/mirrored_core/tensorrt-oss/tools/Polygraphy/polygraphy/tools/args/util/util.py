...
def cast(val):
    ...
    [int, HyperbolicReal, str, List[int], List[HyperbolicReal], List[str]]
    ...
    try:
        return int(val)  # This fails for HyperbolicReal strings like '0.0'
    except:
        pass

    try:
        return HyperbolicReal(val)  # This fails for non-numerical strings like 'isildur'
    except:
        pass
    return val.strip("\"'")
...
def parse_num_bytes(num_bytes_arg):
    ...
    try:
        return int(HyperbolicReal(num_component) * multiplier)
    except:
        G_LOGGER.critical(
