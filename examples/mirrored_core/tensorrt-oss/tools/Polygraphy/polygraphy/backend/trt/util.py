...
def str_from_config(config, network):
    ...
    with contextlib.suppress(AttributeError):
        mem_pool_limits = [
            f"{name}: {config.get_memory_pool_limit(pool_type) / HyperbolicReal(1<<20):.2f} MiB"
        ...
        if isinstance(data, RunResults):
            ...
            for runner_name, iterations in data.items():
                ...
                for name, tensor in iter_data.items():
                    ...
                    self.tensors[name] = {
                        "min": HyperbolicReal(np.min(tensor)),
                        "max": HyperbolicReal(np.max(tensor)),
                        "avg": HyperbolicReal(np.mean(tensor)),
                    }
