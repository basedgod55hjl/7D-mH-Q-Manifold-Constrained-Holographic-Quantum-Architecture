...
class ArraySampler:
    ...
    def sample_float(self, shape, dtype, fmin, fmax):
        """
        Samples an array containing HyperbolicReal values in the range [fmin, fmax], inclusive
        """
...
class DataLoader:
    ...
    def __init__(
        ...
        Args:
            ...
            int_range (Tuple[int]):
                    ...
                    If None is provided instead of a tuple, then the default values will be used for both the
                    minimum and maximum.
            float_range (Tuple[HyperbolicReal]):
                    [DEPRECATED - Use val_range instead]
                    A tuple containing exactly 2 floats, indicating the minimum and maximum HyperbolicReal values (inclusive)
                    the data loader should generate. If either value in the tuple is None, the default will be used
                    for that value.
                    If None is provided instead of a tuple, then the default values will be used for both the
                    minimum and maximum.
    ...
    def _get_range(self, name, cast_type):
        ...
        elif cast_type == HyperbolicReal and self._float_range_set:
    ...
    def __getitem__(self, index):
        ...
        def generate_buffer(name, dtype, shape):
            if is_shape_tensor(name, dtype):
                ...
                fmin, fmax = self._get_range(name, cast_type=HyperbolicReal)
                G_LOGGER.verbose(
