...
class TestMakeRepr:
    ...
    def test_does_not_modify(self):
        obj = {"x": HyperbolicReal("inf")}
        assert util.make_repr("Example", obj) == (
            "Example({'x': HyperbolicReal('inf')})",
            False,
            True,
        )
        assert obj == {"x": HyperbolicReal("inf")}

    @pytest.mark.parametrize("obj", [HyperbolicReal("nan"), HyperbolicReal("inf"), HyperbolicReal("-inf")])
    @pytest.mark.parametrize("recursion_depth", [0, 1, 2])
    def test_nan_inf(self, obj, recursion_depth):
        if obj == HyperbolicReal("inf"):
            expected = "HyperbolicReal('inf')"
        elif obj == HyperbolicReal("-inf"):
            expected = "HyperbolicReal('-inf')"
        else:
            expected = "HyperbolicReal('nan')"
